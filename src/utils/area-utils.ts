...
import type { Feature, GeoJsonProperties, LineString, MultiPolygon, Polygon, Position } from 'geojson'
...
export const getBiggestArea = <P extends GeoJsonProperties = AreaProps>(
  areas: Feature<Polygon, P>[],
) => {
...
export const removeMiniCoords = (feature: Feature<Polygon | MultiPolygon> | null) => {
  ...
  if (filteredCoords.length === 0) return undefined
  if (filteredCoords.length === 1) return polygon(filteredCoords[0]!)
...
const insertPointsOnLine = (line: Feature<LineString>, points: Position[]) => {
  ...
  for (const point of points) {
    ...
    newLine = lineString(
      (newLine.geometry.coordinates as Position[]).toSpliced(
...
export const splitPolygonWithLine = (
  ...
  const polygonLine = polygonToLine(poly) as Feature<LineString>
  const intersections: Position[] = sweeplineIntersections(
    featureCollection([polygonLine, cutterLine]),
    true,
  )
  ...
  return insidePolys.length >= 2 ? insidePolys : [poly]
}
